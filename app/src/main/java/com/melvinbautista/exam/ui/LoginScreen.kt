...
fun LoginScreen(
    ...
    LaunchedEffect(authentication?.auth) {
        when (authentication?.auth) {
            States.LoggedIn.SUCCESS -> navController.navigate(NavigationScreens.Home.route)
            States.LoggedIn.FAILED -> Toast.makeText(
                context,
                Messages.Toast.INVALID_LOGIN,
                Toast.LENGTH_SHORT
            )
                .show()
        }
    ...
    Scaffold(topBar = {
        ...
        Column(
            ...
            TextButton(onClick = { navController.navigate(NavigationScreens.Register.route) }) {
